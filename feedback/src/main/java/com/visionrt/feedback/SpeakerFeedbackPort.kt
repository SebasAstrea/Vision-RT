 ...
 *
 * Tracks the priority of the in-flight utterance so [com.visionrt.core.orchestration.AlertPipeline]
 * can avoid interrupting a CRITICAL alert with another CRITICAL alert
 * (which would cut the phrase mid-word). Lower-priority alerts are still
 * preempted by critical ones (FR-006.3 / OR-009).
 */
